 ...
 * @param cwd
 * @param pkg
 * @returns eslint-config-fibonacci/index
 * @returns eslint-config-fibonacci/react
 * @returns eslint-config-fibonacci/typescript/index
 * @returns eslint-config-fibonacci/typescript/react
 */
const getConfigType = (cwd: string, pkg: PKG): string => {
  ...
  if (reactFiles.length > 0 || dependencies.some((name) => /^react(-|$)/.test(name))) {
    ...
    dsl = 'vue';
  } else if (dependencies.some((name) => /^taro(-|$)/.test(name))) {
    dsl = 'taro';
  }

  return 'eslint-config-fibonacci/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '');
};
