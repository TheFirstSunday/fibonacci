...
import markdownLintConfigFibonacci from 'markdownlint-config-fibonacci';
import markdownLintRuleHelpers from 'markdownlint-rule-helpers';
import type { ScanOptions, ScanResult } from '../types';

type LintOptions = markdownLint.Options & { fix?: boolean, resultVersion?: number };
...
export const getLintConfig = (opts: ScanOptions): LintOptions => {
  ...
  if (lintConfigFiles.length === 0) {
    lintConfig.config = markdownLintConfigFibonacci;
  } else {
